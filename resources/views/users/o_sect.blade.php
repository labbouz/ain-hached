            ...
            <div class="col-lg-12 col-md-12 col-sm12 col-xs-12">
                <div class="page-title">
                    ...
                    <div id="header_index" class="header_action" dir="rtl">
                        ...
                        <h3> @lang('users.list_users_roles_observateur_sectoriels') {{ $secteur->nom_secteur }}   </h3>
                        <p class="text-muted">@lang('users.desc_list_users_roes') <strong>{{ $role->name }} @lang('users.pour_secteur') {{ $secteur->nom_secteur }} </strong>  @lang('users.count_users') <strong>(<span class="count" >{{ $role->users->count() }}</span>)</strong> @lang('users.observateur').</p>
                    </div>
...
@section('url_ajax')
    ...
    --}}
    <input id="index" type="hidden" value="{{ route('json.users.index', ['id_role' => $role->id, 'id_inicateur' => $secteur->id] ) }}">
