            ...
            <div class="col-lg-12 col-md-12 col-sm12 col-xs-12">
                <div class="page-title">
                    ...
                    <div id="header_index" class="header_action">
                        ...
                        <h3> @lang('abus.accrochages_medias')  </h3>
...
@section('url_ajax')
    <input id="index" type="hidden" value="{{ route('json.accrochages_medias.index', $abus->id) }}">
    <input id="store" type="hidden" value="{{ route('accrochages_medias.store') }}">
                ...
                <div class="form-box box">
                    <form autocomplete="off" class="form-cart" dir="rtl" data-error="@lang('main.info_monquant')" data-id="{id}">
                        <div class="form-group">
                            <select id="media_id" name="media_id" class="form-control" data-reset="{media_id}">
                                <option value="0">@lang('main.selectionnez')  @lang('abus.accrochages')</option>
                                @foreach ($medias as $media)
                                    <option value="{{ $media->id }}">{{ $media->categorie_media->nom_categorie_media }} {{ $media->nom_media }}</option>
                                @endforeach
                            </select>
                        </div>
...
@section('footer')
    <script src="{{ asset('js/management_accrochages_medias.js') }}"></script>
@endsection