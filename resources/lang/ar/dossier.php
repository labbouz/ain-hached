<?php

return [

    'add_file' => 'إنشاء ملف جديد',
    'files_non_lus' => 'ملفات حديثة',
    'files_important' => 'ملفات مهمة',
    'files_region' => 'ملفات الجهة',
    'files_show' => 'عرض الملفات',
    'myfiles' => 'ملفاتي',
    'exactions' => 'الإنتهاكات',
    'search_dossier' => 'البحث في الملفات',

    'description_add_file' => 'إنشاء ملف جديد عبر تحديد المؤسسة النقابية',
    'description_dashboard' => 'العودة إلى لوحة التحكم ',
    'description_files_non_lus' => 'الملفات التي لم تتجاوز مدة شهر منذ تاريخ إنشائها',
    'description_files_important' => 'الملفات التي تحتوي على الانتهاكات الخطيرة جدا',
    'description_myfiles' => 'الملفات التي قد قمت بإضافتها في النظام',
    'description_files_show' => 'عرض جميع الملفات التي تندرج تحت صلاحياتي',
    'description_exactions' => 'عرض جميع الملفات التي تندرج تحت صلاحياتي حسب الإنتهاكات والمواجهات المرتبطة',
    'description_search_dossier' => 'البحث في جميع الملفات التي تندرج تحت صلاحياتي حسب المعلومات المرتبطة بالملف من معلومات عن المؤسسة النقابية عن المتضرر والمعتدي',


    'dossiers' => ' الملفات',
    'description_dossiers' => 'الملفات المدرجة في النظام',
     'societe' => 'أسم المؤسسة',
     'secteur' => 'القطاع',
     'gouvernorat' => 'الجهة',
     'add_dossier' => 'إضافة ملف جديد',
     'fait_select_societe' => 'قم بأختيار المؤسسة النقابية',
     'select_societe' => 'أختيار المؤسسة النقابية',
     'select_determine' => 'حدد التصنيف',
    'type_societe' => 'تصنيف المؤسسة',
     'add_dossier_v' => 'إضافة إنتهاك  جديد',


     'info_societe' => 'المؤسسة التي وقع فيها الانتهاك',
     'cdi_oui' => 'نعم',
     'cdi_non' => 'لا',


     'les_violation' => 'الانتهاكات',


    /*** creat dosser ***/
    'cpanel_dossier' => 'لوحة تحكم الملفات',
    'choizir_secteur' => 'قم بإختيار القطاع التي تنتمي إليها المؤسسة المنتهكة',
    'description_creation_dossier' => 'لضمان فعالية النظام ولإصدار الإحصائيات الصحيحة يجيب التأكد من القطاع والمعتمدية التي تنتمي إليهما المؤسسة النقابية قبل إنشاء الملف أو المؤسسة المنتهكة',
    'description_creation_dossier_2' => 'في حالة عدم وجود المؤسسة النقابية المراد البحث عنها في القائمة أدناه أو إضافة مؤسسة جديدة. عليك التوجه إلى إدارة المؤسسات النقابية لإضافة المؤسسة النقابية في المنظومة ومن ثم تستطيع الولوج إلى إدارة الملفات لهذه المؤسسة.',
    'choizir_gouvernorat' => 'قم بتحديد الولاية التي تنتمي إليها المؤسسة النقابية',

    'choizir_delegation' => 'قم بتحديد المعتمدية التي تنتمي إليها المؤسسة المنتهكة',
    'choizir_societe' => 'قم بتحديد المؤسسة المنتهكة لإنشاء ملف جديد',
    'search_societes' => 'البحث عن المؤسسة المنتهكة',
    'not_exit' => 'لاتوجد أية مؤسسات منتهكة تنتمي إلى ',
    'not_exit_secteur' => 'قطاع',
    'not_exit_delegation' => 'بمعتمدية',
    'not_exit_gouvenorat' => 'ولاية',

    'search_in' => 'يتم البحث في',
    'and' => 'و',
    'search_in_secteur_delegation' => 'في القطاع والمعتمدية المحددتين سابقا',
    'are_you_sure' => 'هل أنت واثق؟',
    'confirmButtonText' => 'نعم، قم بإنشاء الملف',

    'description_are_you_sure' => 'هل أنت متأكد من إنشاء ملف جديد للمؤسسة المنتهكة',
    'description_are_you_sure_secteur' => 'والتي تنتمي لقطاع',

    'message_save_succes_dossier' => 'لقد تم إنشاء الملف رقم',
    'message_desc_save_succes_dossier' => ' تمت إضافة ملف جديد للمؤسسة المنتهكة',
    'pour_societe' => 'للمؤسسة النقابية',
    'histrory_dossier' => 'عرض ملخص شامل لجميع الأحداث المتعلقة بهذا الملف',
    'histrory_dossier_violations' => 'إدارة الإنتهاكات المدرجة في هذا الملف والبالغ عددها',

    'detail_historique_dossier' => 'عرض ملخص شامل لجميع الأحداث المتعلقة بالملف رقم',

    'suppression_defenitife_dossier' => 'عملية حذف هذا الملف من النظام ... هي بمثابة حذف جميع الإنتهاكات و المواجهات والوثائق المرتبطة بهذا الملف... مع العلم إن من يملك صلاحيات هذه العملية هو ناشئ الملف أو أحد المشرفين العامين في المنظومة',

    'etes_vous_sure' => 'هل أنت واثق من حذف الملف رقم',
    'message_delete_succes_dossier' => 'تم حذف الملف  رقم ',
    'from_systeme' => 'من النظام',
    'management_societe' => 'إدارة المؤسسات المنتهكة التي تنتمي لقطاع',


    'dossier_number' => 'الملف رقم',
    'resume_dossier' => 'خلاصة الملف',
    'societe_secteur' => 'هذه المؤسسة المنتهكة تنتمي لقطاع',
    'societe_pour_delegation' => 'بمعتمدية',
    'societe_pour_gouvenorat' => 'ولاية',



    'observateurs_concernees' => 'الراصدون المعنيون  بالملف',


    'display_indicateur_gravite_danger' => 'عدد الإنتهاكات الخطيرة جدا المدرجة في هذا الملف',
    'display_indicateur_gravite_warning' => 'عدد الإنتهاكات الخطيرة المدرجة في هذا الملف',
    'display_indicateur_abus_gestion' => 'إدارة جميع الإنتهاكات المدرجة في هذا الملف والبالغ عددها',

    'display_indicateur_confrontations_medias' => 'العدد الكلي لمواجهات الإعلام ضد الإنتهاكات المدرجة في هذا الملف',
    'display_indicateur_confrontations_plaintes' => 'العدد الكلي لمواجهات الشكاوي ضد الإنتهاكات المدرجة في هذا الملف',
    'display_indicateur_confrontations_mouvements' => 'العدد الكلي لمواجهات التحركات ضد الإنتهاكات المدرجة في هذا الملف',

    'management_abus' => 'إدارة جميع الإنتهاكات المدرجة في هذا الملف',


    'info_societe_principale' => 'المعلومات الأساسية للمؤسسة المنتهكة',
    'close_popup' => 'إغلاق',
    'not_info' => 'المعلومة غير مدرجة',
    'conventions_societe' => 'الاتفاقيات التي تخص هذه المؤسسة',
    'card_observateur' => 'بطاقة الراصد',
    'send_observateur' => 'مراسلة راصد',
    'send_l_observateur' => 'مراسلة ال',

    'sujet_send' => 'موضوع المراسلة',
    'sujet_dossier_numer' => 'فيما يخص الملف رقم',
    'pour_societe_agressif' => 'للمؤسسة المنتهكة',
    'the_send' => 'المراسلة',
    'send' => 'قم بالإرسال',
    'piece_jointe' => 'الوثائق المرفقة',
    'succes_send_message' => 'لقد تمت عملية الإرسال بنجاح في إنتظار الرد من المرسل إليه',
    'sujet_societe' => 'فيما يخص مؤسسة المنتهكة',
    'observateurs_concernees' => 'الراصدون المعنيون  بالمؤسسة المنتهكة',
    '' => '',
    '' => '',
    '' => '',
    '' => '',
    '' => '',
    '' => '',


];
